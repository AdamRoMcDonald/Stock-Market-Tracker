...
public class APIAccessor {
    ...
    public void infoLoader(ArrayList<String> ticker, String startDate, String endDate, StockHashTable centralTable) throws IOException {
...
//            System.out.println("Past URL ");
    ...
    private void parseJsonResponse(String responseBody, StockHashTable centralTable, ArrayList<String> tickers) {
        JSONObject json = new JSONObject(responseBody);

        // Case 1: Single symbol response
        if (json.has("meta")) {
            parseSymbol(json, centralTable);
            infoScreen.infoPrintScreen(infoScreen.tArea, this, centralTable, tickers);
        }
    ...
    private void parseSymbol(JSONObject json, StockHashTable centralTable) {
        ...
        for (int i = 0; i < values.length(); i++) {
            ...
            if(centralTable.get(key) != null && centralTable.printOne(key).contains(symbol)){
                ...
                System.out.println(centralTable.printOne(key));
